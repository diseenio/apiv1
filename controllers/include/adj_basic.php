...
$app->get("/".$tabla."", function () use ($app, $db, $tabla, $pk) {
    ...
    echo json_encode($db->$tabla()->where("estado='V' ")->order("$pk")); //->where("estado='V'")->order("orden")
});
...
$app->get("/".$tabla."/filtro/:filtro+", function ($filtro) use ($app, $db, $tabla, $pk) {
  // ejemplo: /api.php/notas/filtro/id/>/1130
    ...
    $where = str_replace(","," ",$where." and estado = 'V' "); // . " and estado='V' "
    $res = $db->$tabla->where($where)->order("$pk"); //->order("orden")
    echo json_encode($res);
});
